...
function selectGame(gameType) {
  ...
  document.querySelectorAll("[data-game-card]").forEach((card) => {
    card.classList.toggle(
      "selected",
      card.dataset.gameCard === selectedGameType
    );
  });
}
...
function updateRoom(room) {
  ...
  $("yangSubmission").classList.toggle(
    "hidden",
    isLiarGame || room.status !== "waiting"
  );
  $("yangGuessBox").classList.toggle(
    "hidden",
    isLiarGame || room.status !== "playing"
  );
  $("votePanel").classList.toggle("hidden", !isLiarGame);
  $("yangBoard").classList.toggle(
    "hidden",
    isLiarGame || room.status === "waiting"
  );
  $("finishYangGame").classList.toggle(
    "hidden",
    isLiarGame || room.status !== "playing"
  );
...
function updateMe(info) {
  ...
  if (
    ...
    if (me.yangSolved) {
      ...
      setMessage(
        yangGuessMessage,
        "정답입니다! 이제 다른 사람이 맞히는 걸 지켜보세요."
      );
    } else if (me.yangGuessResult === "wrong") {
      setMessage(
        yangGuessMessage,
        `마지막 추측 “${me.yangGuess}”은(는) 아니에요.`
      );
    } else {
...
function renderMyCard() {
  ...
  if (currentRoom.status === "waiting") {
    ...
    if (currentRoom.gameType === "yang") {
      detail.textContent =
        "제시어를 입력하고 기다려 주세요. 모든 사람이 제출하면 방장이 시작할 수 있어요.";
    } else {
  ...
  if (currentRoom.gameType === "yang") {
    ...
    title.textContent = me.yangSolved
      ? `정답 확인: ${me.yangGuess}`
      : "내 제시어는 비공개!";
  ...
  if (me.role === "liar") {
    ...
    if (currentRoom.foolMode && me.word) {
...
function renderPlayers(players) {
  ...
  players.forEach((player) => {
    ...
    if (currentRoom?.gameType === "yang" && currentRoom.status === "waiting") {
      readyBadge = player.hasYangSubmission
      ...
      readyBadge = player.yangSolved
        ? '<span class="badge ready">정답</span>'
        : '<span class="badge">추리 중</span>';
    }
    ...
    li.innerHTML = `
      ...
      <span>
        ...
        ${player.id === socket.id ? '<span class="badge me">나</span>' : ""}
        ${readyBadge}
      </span>
    `;
...
function renderVoteArea() {
  ...
  currentRoom.players.forEach((player) => {
    ...
    const voteCount = Object.values(currentRoom.votes || {}).filter(
      (id) => id === player.id
    ).length;
    ...
    card.innerHTML = `
      <span>
        ...
        <span class="badge">${voteCount}표</span>
      </span>
    `;
...
function renderYangBoard() {
  ...
  (currentRoom.yangBoard || []).forEach((item) => {
    ...
    const solvedBadge = playerState?.yangSolved
      ? ' <span class="badge ready">정답</span>'
      : "";
    ...
    card.innerHTML = `
      <span class="player-name">
        ...
        ${isMine ? ' <span class="badge me">나</span>' : ""}
        ${solvedBadge}
      </span>

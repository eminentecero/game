...
let me = null;
let showResultWord = false; // 정답 키워드 보여주기 코드
...
function renderPlayers(players) {
  ...
  players.forEach((player) => {
    ...
    li.innerHTML = `<span class="player-name">${escapeHtml(
      player.nickname
    )}</span>${player.isHost ? '<span class="badge">방장</span>' : ""}${
      player.id === socket.id ? '<span class="badge">나</span>' : ""
    }`;
    list.appendChild(li);
  });
}
...
function renderVoteArea() {
  ...
  currentRoom.players.forEach((player) => {
    ...
    card.innerHTML = `<span>${escapeHtml(
      player.nickname
    )} <span class="badge">${voteCount}표</span></span>`;
